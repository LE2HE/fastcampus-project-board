...
class JpaRepositoryTest {
    ...
    void givenArticleCommentHavingChildComments_whenDeletingParentComment_thenDeletesEveryComment() {
        ...
        // then
        assertThat(articleCommentRepository.count()).isEqualTo(previousArticleCommentCount - 5); // 5 = 테스트 댓글 + 대댓글 4개
    }

    @DisplayName("댓글 삭제와 대댓글 전체 연동 삭제 테스트 - 댓글 ID + 유저 ID")
    @Test
    void givenArticleIdHavingChildCommentsAndUserId_whenDeletingParentComment_thenDeletesEveryComment() {
        // given
        long previousArticleCommentCount = articleCommentRepository.count();
